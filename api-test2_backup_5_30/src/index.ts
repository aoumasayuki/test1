...
app.get("/graph/view", async (c) => {
  ...
  const logs = await prisma.phaseLog.findMany({ where: { game: "人狼" }, orderBy: { startTime: "asc" } });
  const selected = logs.find(l => `${l.gameDate}-${l.gamePhase}` === phaseKey);
  let heartRates = [];

  if (selected) {
    heartRates = await prisma.csvData.findMany({
      where: {
        id,
        Timestamp: {
          gte: selected.startTime,
          lte: selected.endTime || undefined,
        },
      },
  ...
  let heartRateData: { x: string, y: number | null }[] = [];
  for (let i = 0; i < heartRates.length; i++) {
    const current = heartRates[i];
    const next = heartRates[i + 1];
    heartRateData.push({ x: current.Timestamp.toISOString(), y: current.Heart_Rate });

    if (next) {
      const diffSeconds = (new Date(next.Timestamp).getTime() - new Date(current.Timestamp).getTime()) / 1000;
      if (diffSeconds > 1) {
        heartRateData.push({ x: next.Timestamp.toISOString(), y: null }); // 隙間を挿入
      }
    }
  }
      ...
      <script>
        ...
        new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              x: { title: { display: true, text: 'Time' } },
              y: { title: { display: true, text: 'Heart Rate (BPM)' }, min:70 }
            }
          }
        });
      </script>
